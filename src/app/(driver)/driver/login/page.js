...
function parseError(message) {
  ...
  if (lower.includes("forbidden") || lower.includes("cannot access")) return "This account is not authorized for the driver app.";
...
export default function DriverLoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await login(email, password, "/auth/driver/login");
      ...
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
          ...
          <div className="flex items-center justify-end text-xs">
            <Link href="/driver/forgot-password" className="text-primary hover:underline">Forgot password?</Link>
          </div>
